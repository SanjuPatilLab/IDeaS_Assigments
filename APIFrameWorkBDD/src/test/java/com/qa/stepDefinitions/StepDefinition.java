...
public class StepDefinition extends Utils{
	...
	public void user_calls_api_with_http_request(String resource,String method) {
		...
		Utils.responseSp = new ResponseSpecBuilder()
				.expectStatusCode(200)
				.expectContentType(ContentType.JSON)
				.build();

		APIMethods apiResponse = new APIMethods(method.toUpperCase(),apiResource.getResourcePath(),Utils.requestSp);

		Utils.response = apiResponse.getResponse();
	...
	public void the_api_call_is_success_with_status_code(Integer statusCode) {
		...
		assertTrue("Status code not matching : Expected :"+ statusCode +" , Actual"+ Utils.response.getStatusCode(), statusCode.equals(Utils.response.getStatusCode()));
	...
	public void in_response_body_is(String key, String expectedValue) {
		...
		String actualValue = getJsonPathValue(Utils.response, key);
	...
	public void verify_place_id_is_created_which_maps_to_using_api(String expectedName, String resource) throws Exception {
		...
		Utils.requestSp = given().spec(requestSpecification()).queryParam("place_id", place_id);
	...
	public void delete_place_payload() throws Exception {
		...
		Utils.requestSp = given().spec(requestSpecification())
